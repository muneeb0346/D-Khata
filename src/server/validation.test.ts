...
describe('validateCustomerData', () => {
  it('name with digits → invalid', () => {
    const result = validateCustomerData({ name: 'Ali123', phone: '03001234567' });
  ...
  it('CNIC empty/undefined → valid (optional)', () => {
    ...
    expect(validateCustomerData({ name: 'Ali Khan', phone: '03001234567', cnic: '' })).toBeNull();
  });
...
describe('CNIC validation via validateCustomerData', () => {
  it('CNIC 12345-1234567-1 → valid', () => {
    expect(validateCustomerData({ name: 'Ali Khan', phone: '03001234567', cnic: '12345-1234567-1' })).toBeNull();
  });

  it('CNIC too short → invalid', () => {
    const result = validateCustomerData({ name: 'Ali Khan', phone: '03001234567', cnic: '12345-1234567' });
    expect(result).toEqual({ ok: false, error: 'CNIC must follow the format xxxxx-xxxxxxx-x.' });
  });

  it('CNIC wrong dash positions → invalid', () => {
    const result = validateCustomerData({ name: 'Ali Khan', phone: '03001234567', cnic: '1234-1234567-1' });
    expect(result).toEqual({ ok: false, error: 'CNIC must follow the format xxxxx-xxxxxxx-x.' });
  });
});
